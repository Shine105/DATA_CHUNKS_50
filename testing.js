...
// Initialize output data array to store data from all files
let outputData = [['Zone', 'Name of Station', 'Date', 'Time', 'SCADA Tag', 'Data', 'Range']];
  ...
  function addDataToOutput(rowValues, startRow) {
    rowValues.forEach(({ value, col }) => {
      ...
      columnData.forEach((dataValue, index) => {
        outputData.push(['HSN', nameOfStation, date, timeIntervals[index], value, dataValue, `${startRow}-${startRow + 1440}`]);
      });
    });
  }

  // Add the data from the third row and 1440 rows of SCADA tag data
  addDataToOutput(thirdRowValues, 3); // Starts from the 4th row (index 3)
}

// The location of folder containing the input files
const inputFolder = './HSN_15102023';

// Here we read all files in the folder and process each one
fs.readdir(inputFolder, (err, files) => {
  ...
  for (let batchIndex = 0; batchIndex < batchCount; batchIndex++) {
    outputData = [['Zone', 'Name of Station', 'Date', 'Time', 'SCADA Tag', 'Data', 'Range']]; // Reset outputData for each batch
    ...
    const outputFilePath = path.join('output', `Batch_${batchIndex + 1}_Extracted_SCADA_Tag_Data.xlsx`);
